...
export default function Feed() {
  ...
  const getPhotoURL = (photoPath) => {
    ...
    return photoPath.startsWith('http') ? photoPath : `${backendURL}${photoPath}`;
  };
        ...
        {posts.map((p) => (
          <article key={p._id} className="card">
            <div className="userrow">
              ...
              <div className="row">
                <button className="btn small" onClick={() => like(p._id)}>
                  ❤ {p.likes.length}
                </button>
                <button className="btn small ghost" onClick={() => startEdit(p)}>
                  Edit
                </button>
                <button className="btn small ghost" onClick={() => remove(p._id)}>
                  Delete
                </button>
              </div>
            </div>
    ...
    <form onSubmit={submit}>
      ...
      {preview && (
        <img
          src={preview}
          alt="Preview"
          style={{ marginTop: 8, maxHeight: 150, objectFit: 'cover' }}
        />
      )}
      <div style={{ height: 12 }} />
...
function EditModal({ post, onClose, onSave }) {
  ...
  const [photo, setPhoto] = React.useState(null);
  const [preview, setPreview] = React.useState(post.photo || null);
  ...
  const handlePhotoChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => setPreview(reader.result);
      reader.readAsDataURL(file);
    } else setPreview(post.photo || null);
  };
        ...
        <form onSubmit={submit}>
          ...
          {preview && (
            <img
              src={preview}
              alt="Preview"
              style={{ marginTop: 8, maxHeight: 200, objectFit: 'cover' }}
            />
          )}
          <div style={{ height: 12 }} />
