...
export default function Feed() {
  ...
  useEffect(() => {
    ...
    if (input) input.addEventListener('input', e => setQuery(e.target.value));
  ...
  // Like, comment, delete
  const like = async id => {
    try { await api.post('/posts/' + id + '/like'); load(query); } 
    catch (err) { alert(err.response?.data?.message || err.message); }
  };
  const comment = async (id, text) => {
    if (!text) return;
    try { await api.post('/posts/' + id + '/comment', { text }); load(query); } 
    catch (err) { alert(err.response?.data?.message || err.message); }
  };
  const remove = async id => {
    if (!confirm('Delete this post?')) return;
    try { await api.delete('/posts/' + id); load(query); } 
    catch (err) { alert(err.response?.data?.message || err.message); }
  };
        ...
        {posts.map(p => (
          <article key={p._id} className="card">
            <div className="userrow">
              <img className="dp" src={p.author.dp || 'https://via.placeholder.com/52'} />
              <div style={{ flex: 1 }}>
                <Link to={'/profile/' + p.author._id} className="huge" style={{ fontSize: 20, textDecoration: "none" }}>
                  {p.author.name}
                </Link>
            ...
            {p.photo && <img className="post-photo" src={p.photo} />}
      ...
      <aside className="sidebar">
        <div className="card">
          ...
          <p className="meta" style={{ marginTop: 8 }}>
            Click edit to modify your posts. Use the search box at top for instant results.
          </p>
        </div>
        <div className="card">
